...
const lines = [
  ...
  '# Mail one alert per scored event at MEDIUM/HIGH/CRITICAL, to the customer the',
  '# event belongs to. SMTP creds are secrets and are NOT printed here — export',
  '# EMAIL_USER / EMAIL_PASS yourself or NOTHING is sent: with either blank there',
  '# is no transport and alerts + OTPs only appear as [DEV MAIL] log lines.',
  '# RISK_ALERT_FROM / _FALLBACK_TO are intentionally NOT exported: blank makes',
  '# both default to EMAIL_USER, the only address the SMTP account is allowed to',
  '# send as. Override only with a verified Send-As alias.',
  'export RISK_ALERT_ENABLED=true',
  'export RISK_ALERT_MIN_LEVEL=MEDIUM',
